class DataBus {
  constructor() {
    ...
    console.log('DataBus instance created');
  }
  ...
  async loadRules(rulesPath = './rules.json') {
    try {
      ...
      console.log('DataBus rules loaded:', this.rules.version);
    } catch (error) {
    ...
    return {
      ...
      mfeDataAccess: {
        ...
        "mfe-reports": { 
          readable: ["state"], 
          writable: ["state"],
          description: "Reports MFE can read and update state only"
        }
      },
      dataValidation: {},
      defaultData: {}
  ...
  async initialize(initialData = {}, rulesPath) {
    ...
    console.log('DataBus initialized with data:', this.data);
  }
  ...
  updateData(mfeName, updates) {
    ...
    this.notifyAllMFEs();
    console.log(`DataBus updated by ${mfeName}:`, validatedUpdates);
  }
  ...
  async reloadRules(rulesPath) {
    await this.loadRules(rulesPath);
    console.log('DataBus rules reloaded');
  }
}
